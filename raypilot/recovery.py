...
class RecoveryController:
    def __init__(self, warn_cov=0.13, off_cov=0.10, recover_cov=0.15,
                 slow_throttle=0.10, reverse_throttle=-0.15, control_hz=20.0,
                 offtrack_secs=0.5, stop_secs=0.3, max_reverse_secs=3.0, recover_secs=0.3,
                 stuck_secs=1.0, pulse_len=3, pulse_gap=2, reverse_steer_mode="align",
                 align_gain=1.0, align_thr=0.30):
        ...
        # "align" (rotate parallel to the track) | "hold" (retrace) | "mirror" | "straight"
        self.reverse_steer_mode = reverse_steer_mode
        self.align_gain, self.align_thr = align_gain, align_thr
        self.reset()
    ...
    def step(self, coverage, steer, throttle):
        ...
        # ---- REVERSE: back up while ROTATING the car parallel to the track, until aligned+visible ----
        if self.state == REVERSE:
            # in "align" mode also require the heading to be ~forward (car parallel) before resuming;
            # other modes resume on coverage alone
            aligned = abs(steer) < self.align_thr if self.reverse_steer_mode == "align" else True
            if coverage >= self.recover_cov and aligned:   # track back AND aligned -> SUDDEN STOP, settle
            ...
            self._rec = 0                       # not there yet -> keep backing/rotating (back-and-forth)
            self._t += 1
            ...
            if self.reverse_steer_mode == "align":
                # mirror the LIVE heading: reversing with opposite steer swings the nose TOWARD the
                # track, so the car rotates parallel; as it aligns, |steer|->0 and it stops rotating
                rsteer = max(-1.0, min(1.0, -self.align_gain * steer))
            else:
                rsteer = self._reverse_steer()
            backing = (self._pulse_i % (self.pulse_len + self.pulse_gap)) < self.pulse_len
            self._pulse_i += 1
            return rsteer, (self.reverse_throttle if backing else 0.0), REVERSE
